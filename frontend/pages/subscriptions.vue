<template>
    <div class="p-6 max-w-[1400px] mx-auto">
      ...
      <Dialog v-model:visible="showModal" modal :header="editingId ? '✏️ Edit Subscription' : '📅 Add New Subscription'" :style="{ width: '450px' }">
  ...
  // Sayfa açıldığında verileri Django'dan çek
  onMounted(() => {
    loadSubscriptions()
  })
  ...
  const editingId = ref<number | null>(null) // Eğer bu null değilse "Düzenleme" modundayız demektir
  ...
  // "Ekle" butonuna basıldığında formu temizle ve aç
  const openAddModal = () => {
  ...
  // "Kalem (Düzenle)" butonuna basıldığında o aboneliğin verilerini forma doldur
  const openEditModal = (sub: any) => {
    editingId.value = sub.id
    formData.value = { 
      name: sub.name, 
      amount: sub.amount, 
      dueDate: new Date(sub.dueDate) // Takvimin okuyabilmesi için Date objesi olarak verdik
    }
    showModal.value = true
  }
