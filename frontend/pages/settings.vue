<template>
    <div class="p-6 max-w-[1400px] mx-auto">
      ...
      <Dialog v-model:visible="showPasswordDialog" modal header="🔒 Change Password" :style="{ width: '450px' }">
  ...
  import InputNumber from 'primevue/inputnumber' // YENİ EKLENDİ
  ...
  // --- YENİ: BÜTÇE İŞLEMLERİ ---
  const isSavingBudgets = ref(false)
  // Kullanıcının ekranda göreceği/gireceği bütçeler (Kendi para biriminde)
  const budgets = ref({
  ...
  // Sayfa yüklendiğinde LocalStorage'dan bütçeleri al
  onMounted(() => {
    const savedBudgets = localStorage.getItem('budgetLimits')
    if (savedBudgets) {
      const parsed = JSON.parse(savedBudgets)
      // Veritabanı/LocalStorage mantığında hep baz kuru (USD) saklıyoruz
      // Ekranda gösterirken kullanıcının güncel kuruna çeviriyoruz
      const rate = getRate()
      budgets.value = {
  ...
  // Bütçeleri kaydetme işlemi
  const saveBudgets = () => {
    isSavingBudgets.value = true
    
    // Kaydederken rakamları tekrar ana para birimine (USD) bölüp saklıyoruz ki
    // Kullanıcı dövizi değiştirirse doğru çevrilebilsin.
    const rate = getRate()
    const baseBudgets = {
  ...
  //  DİĞER FONKSİYONLAR (DÖVİZ, TEMA, CSV, ŞİFRE) AYNI KALDI 
